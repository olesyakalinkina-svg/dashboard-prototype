...
const matchRows = matches.map(
  (m) =>
    `(${sqlStr(m.id)}, ${m.arenaId}, ${m.opponentId}, ${sqlStr(m.dateSql)}, ${m.attendance}, ${sqlStr(m.status)}, ${sqlStr(m.season)}, ${sqlStr(m.league)}, ${sqlStr(m.tournamentStage)}, ${sqlStr(m.matchClass)}, ${m.ticketSalesWindowDays}, ${sqlNullable(m.series)}, ${m.capacity})`,
);
...
INSERT INTO match (id, arena_id, opponent_id, match_date, attendance, status, season, league, tournament_stage, match_class, ticket_sales_window_days, series, capacity) VALUES
${matchRows.join(",\n")};
