...
export function MatchSalesTable({
  ...
  const maxValues = useMemo(
    () => ({
      ...
      loyaltyDiscountPct: Math.max(
        ...data.map((row) => row.loyaltyDiscountPct),
        0,
      ),
    }),
    [data],
  ...
  const columns = useMemo<ColumnDef<MatchSalesRow, unknown>[]>(
    () => [
      {
      ...
      },
      {
        ...
        cell: ({ getValue }) => (
          <InlineBarCell
            value={getValue<number>()}
            max={maxValues.loyaltyDiscountPct}
            formatted={`${getValue<number>().toFixed(1)}%`}
  ...
  const tableContent = (
    <>
      {embedded && (
        <div className="mb-3 flex justify-end">{searchInput}</div>
      )}
      <table className="w-full text-sm">
        <thead>
          {table.getHeaderGroups().map((hg) => (
